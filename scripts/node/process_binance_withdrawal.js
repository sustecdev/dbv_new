...
function workerHeaders(url) {
    ...
    return { 'X-Worker-Timestamp': ts, 'X-Worker-Signature': generateSignature(u.pathname + u.search, ts) };
}
...
/**
 * Process a single Binance withdrawal by ID
 */
async function processWithdrawalById(withdrawalId) {
    // Try to load from .env first
    await loadConfigFromEnv();

    console.log(new Date().toISOString(), `🚀 Processing Binance withdrawal ID: ${withdrawalId}`);
    ...
    if (!config.vaultAddress || !config.vaultPrivateKey || !config.tokenContract) {
        console.error('❌ Binance configuration missing!');
        console.error('Required environment variables:');
        console.error('  - BSC_VAULT_ADDRESS');
        console.error('  - BSC_VAULT_PRIVATE_KEY');
        console.error('  - BSC_TOKEN_CONTRACT');
        console.error('\nOptional:');
        console.error('  - BSC_RPC_URL (default: https://bsc-dataseed.binance.org/)');
        console.error('  - BSC_CHAIN_ID (default: 56)');
        console.error('  - BSC_GAS_PRICE (default: 5000000000)');
        console.error('  - BSC_GAS_LIMIT (default: 100000)');
        process.exit(1);
    }
    ...
    try {
        ...
        const url = apiEndpoints.get_binance_withdrawals
            ? `${apiEndpoints.get_binance_withdrawals}?status=0`
            : `${apiEndpoints.base}/getBinanceWithdrawals.php?status=0`;
        ...
        const updateUrl = apiEndpoints.update_binance_withdrawals || `${apiEndpoints.base}/updBinanceWithdrawals.php`;
        const { generateSignature } = await import('./secure_worker_client.js');
        const postParams = `id=${withdrawal.id}&status=8`;
        const fullUrl = updateUrl + (updateUrl.includes('?') ? '&' : '?') + postParams;
        const ts = Math.floor(Date.now() / 1000).toString();
        ...
        // Initialize Web3
        console.log(new Date().toISOString(), `🔗 Connecting to BSC network...`);
        ...
        console.log(new Date().toISOString(), `🔗 BSCScan: https://bscscan.com/tx/${txHash}`);
        ...
        if (!receipt) {
            console.log(new Date().toISOString(), `⚠️  Transaction sent but not yet confirmed. Check BSCScan for status.`);
        }
