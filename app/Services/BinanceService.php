 ...
 * Handles interaction with BSC network via JSON-RPC
 */
class BinanceService
{
    ...
    /**
     * Make JSON-RPC call to BSC network
     */
    private function rpcCall(string $method, array $params = []): ?array
    {
        try {
            ...
            if (empty($url)) {
                error_log("BinanceService::rpcCall - RPC URL not configured");
                return null;
            }
            ...
            $ch = curl_init($url);
            if ($ch === false) {
                error_log("BinanceService::rpcCall - Failed to initialize cURL");
                return null;
            }
            ...
            if ($curlError) {
                error_log("BinanceService::rpcCall - cURL error: $curlError");
                return null;
            }
            
            if ($httpCode !== 200 || !$response) {
                error_log("BinanceService::rpcCall - HTTP error: $httpCode, Response: " . substr($response, 0, 200));
                return null;
            }
            
            $result = json_decode($response, true);
            if (json_last_error() !== JSON_ERROR_NONE) {
                error_log("BinanceService::rpcCall - JSON decode error: " . json_last_error_msg());
                return null;
            }
            
            // Check for RPC error in response
            if (isset($result['error'])) {
                error_log("BinanceService::rpcCall - RPC error: " . json_encode($result['error']));
                return null;
            }
            
            return $result['result'] ?? null;
        } catch (Exception $e) {
            error_log("BinanceService::rpcCall - Exception: " . $e->getMessage());
            return null;
        }
    }
    ...
    /**
     * Fetch transaction details from BSC
     */
    public function fetchTransactionDetails(string $txnHash): ?array
    {
        try {
            if (empty($txnHash) || !$this->isValidTransactionHash($txnHash)) {
                error_log("BinanceService::fetchTransactionDetails - Invalid transaction hash: " . substr($txnHash, 0, 20));
                return null;
            }
            
            $txn = $this->rpcCall('eth_getTransactionByHash', [$txnHash]);
            if (!$txn) {
                error_log("BinanceService::fetchTransactionDetails - Transaction not found: " . substr($txnHash, 0, 20));
                return null;
            }
            
            $receipt = $this->rpcCall('eth_getTransactionReceipt', [$txnHash]);
            if (!$receipt) {
                error_log("BinanceService::fetchTransactionDetails - Receipt not found: " . substr($txnHash, 0, 20));
                return null;
            }
            ...
            error_log("BinanceService::fetchTransactionDetails - Exception: " . $e->getMessage());
            return null;
        }
    }
    ...
    public function verifyDeposit(string $txnHash, string $vaultAddress): ?array
    {
        try {
            if (empty($vaultAddress) || !$this->isValidAddress($vaultAddress)) {
                error_log("BinanceService::verifyDeposit - Invalid vault address");
                return null;
            }
            
            $txnDetails = $this->fetchTransactionDetails($txnHash);
            if (!$txnDetails) {
                error_log("BinanceService::verifyDeposit - Failed to fetch transaction details for: " . substr($txnHash, 0, 20));
                return null;
            }
            
            if (!$txnDetails['successful']) {
                error_log("BinanceService::verifyDeposit - Transaction not successful: " . substr($txnHash, 0, 20));
                return null;
            }
            ...
            if (empty($tokenContract)) {
                error_log("BinanceService::verifyDeposit - Token contract not configured");
                return null;
            }
            ...
            if (empty($logs)) {
                error_log("BinanceService::verifyDeposit - No logs found in transaction: " . substr($txnHash, 0, 20));
                return null;
            }
            ...
            foreach ($logs as $log) {
                try {
                    ...
                    if ($amount <= 0) {
                        error_log("BinanceService::verifyDeposit - Invalid amount: $amount");
                        continue;
                    }
                    ...
                    error_log("BinanceService::verifyDeposit - Error processing log: " . $e->getMessage());
                    continue;
                }
            }
            
            error_log("BinanceService::verifyDeposit - No valid transfer found to vault address");
            return null;
        } catch (Exception $e) {
            error_log("BinanceService::verifyDeposit - Exception: " . $e->getMessage());
            return null;
        }
    }
     ...
     * Used when admin marks a manual withdrawal complete.
     */
    ...
    /**
     * Check if address is valid BSC/Ethereum address format
     */
     ...
     * This method provides the data payload for signing
     */
