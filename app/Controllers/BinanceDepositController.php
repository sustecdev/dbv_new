...
class BinanceDepositController
{
    private PDO $pdo;
    ...
    public function postCreate(): void
    {
        ...
        if (!isset($_SESSION['uid'])) {
            Security::logSecurityEvent('binance_deposit_unauthorized', ['ip' => Security::getClientIp()]);
            echo json_encode(['success' => false, 'message' => 'Not authenticated']);
            return;
        }
        
        // Rate limiting
        $rateLimitKey = 'binance_deposit_' . $_SESSION['uid'];
        if (!Security::checkRateLimit($rateLimitKey, 20, 60)) {
            Security::logSecurityEvent('binance_deposit_rate_limit', ['uid' => $_SESSION['uid']]);
        ...
        if (!Security::verifyCsrfToken($csrfToken)) {
            Security::logSecurityEvent('binance_deposit_csrf_failed', ['uid' => $_SESSION['uid']]);
        ...
        if (!$canTransact['allowed']) {
            Security::logSecurityEvent('binance_deposit_verification_failed', [
                'uid' => $uid,
        ...
        if (!$this->binance->isValidTransactionHash($txnHash)) {
            echo json_encode(['success' => false, 'message' => 'Invalid transaction hash format']);
            return;
        }
        
        // Duplicate check
        $stmt = $this->pdo->prepare('SELECT id FROM binance_deposit WHERE txn_hash_bsc = ?');
        ...
        // Verify transaction
        $vaultAddress = $this->cfg['binance']['vault_address'] ?? '';
        if (empty($vaultAddress)) {
            echo json_encode(['success' => false, 'message' => 'Vault address not configured']);
            return;
        }
        
        $depositDetails = $this->binance->verifyDeposit($txnHash, $vaultAddress);
        if (!$depositDetails) {
        ...
        // Calculate value in USD (adjust multiplier as needed)
        $valueUSD = round($amount * 0.01, 2);
        $currencyTo = $_SESSION['currencycode'] ?? 'USD';
        $reason = 'Bridge: Transfer from Binance Smart Chain (' . $this->cfg['binance']['network'] . ')';
        ...
        if ($yemchainBypass) {
            ...
            Logger::warning('YEMChain bypassed for Binance deposit', ['uid' => $uid, 'txn_hash' => $txnHash, 'fake_hash' => $yemHash]);
        } else {
            ...
            if (($resp['status'] ?? 'error') !== 'success') {
                ...
                Logger::error('Binance deposit YEMChain voucher failed', ['uid' => $uid, 'txn_hash' => $txnHash, 'error' => $errorMsg]);
        ...
        // Record deposit (round to 8 decimals for BSC)
        try {
            $amount = round($amount, 8);
            $ins = $this->pdo->prepare('INSERT INTO binance_deposit (uid, txn_hash_bsc, txn_hash_yemchain, amount, status, created_at) VALUES (?, ?, ?, ?, 3, NOW())');
            $ins->execute([$uid, $txnHash, $yemHash, $amount]);
            
            if ($ins->rowCount() === 0) {
                Logger::error('Binance deposit - Failed to insert record', ['uid' => $uid, 'txn_hash' => $txnHash]);
            ...
            Logger::error('Binance deposit - Database error', ['uid' => $uid, 'txn_hash' => $txnHash, 'error' => $e->getMessage()]);
            echo json_encode(['success' => false, 'message' => 'Database error occurred. Please contact support.']);
            return;
        } catch (Exception $e) {
            Logger::error('Binance deposit - Unexpected error', ['uid' => $uid, 'txn_hash' => $txnHash, 'error' => $e->getMessage()]);
