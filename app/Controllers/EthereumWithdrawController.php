...
class EthereumWithdrawController
{
    private PDO $pdo;
    ...
    public function postCreate(): void
    {
        ...
        if (!isset($_SESSION['uid'])) {
            Security::logSecurityEvent('ethereum_withdrawal_unauthorized', ['ip' => Security::getClientIp()]);
            echo json_encode(['success' => false, 'message' => 'Not authenticated']);
            return;
        }
        
        // Rate limiting
        $rateLimitKey = 'ethereum_withdrawal_' . $_SESSION['uid'];
        if (!Security::checkRateLimit($rateLimitKey, 10, 60)) {
            Security::logSecurityEvent('ethereum_withdrawal_rate_limit', ['uid' => $_SESSION['uid']]);
        ...
        if (!Security::verifyCsrfToken($csrfToken)) {
            Security::logSecurityEvent('ethereum_withdrawal_csrf_failed', ['uid' => $_SESSION['uid']]);
        ...
        if (!$canTransact['allowed']) {
            Security::logSecurityEvent('ethereum_withdrawal_verification_failed', [
                'uid' => $uid,
        ...
        if ($this->cfg['app']['debug'] ?? false) {
            Logger::debug('Ethereum withdrawal received', ['uid' => $uid, 'has_pin' => !empty($pin), 'has_key' => !empty($key)]);
        }
        ...
        if (empty($address) || !$this->ethereum->isValidAddress($address)) {
        ...
        if (Security::isBlockedWithdrawalAddress($address, $blockedAddresses)) {
            Security::logSecurityEvent('ethereum_withdrawal_blocked_address_attempted', ['uid' => $uid, 'address_prefix' => substr($address, 0, 12) . '...']);
        ...
        $dailyLimit = $this->cfg['ethereum']['daily_withdrawal_limit'] ?? 0;
        if ($dailyLimit > 0) {
            $bypassDaily = $this->cfg['withdrawal']['daily_limit_bypass_uids'] ?? [];
            $limitCheck = WithdrawalLimits::checkLimit($this->pdo, $uid, 'ethereum', $amount, $dailyLimit, $bypassDaily);
            if (!$limitCheck['allowed']) {
                Logger::warning('Ethereum daily withdrawal limit exceeded', [
                    'uid' => $uid,
                    'amount' => $amount,
        ...
        $feeEnabled = $this->cfg['ethereum']['withdrawal_fee_enabled'] ?? ($this->cfg['withdrawal']['fee_enabled'] ?? true);
        $withdrawalFee = $this->cfg['ethereum']['withdrawal_fee_usdd'] ?? ($this->cfg['withdrawal']['fee_usdd'] ?? 2.50);
        ...
        if ($feeEnabled && !$yemchainBypass) {
            ...
            $feeResp = $this->yem->createVoucher([
                ...
                'currencyCodeTo' => 'USD',
                'reason' => 'Withdrawal fee for Ethereum withdrawal',
            ]);
            ...
            Logger::warning('YEMChain fee bypassed for Ethereum withdrawal', ['uid' => $uid, 'fee' => $withdrawalFee]);
        }
        ...
        if ($yemchainBypass) {
            ...
            Logger::warning('YEMChain bypassed for Ethereum withdrawal', ['uid' => $uid, 'amount' => $amount, 'fake_hash' => $yemHash]);
        } else {
            // Deduct DBV from user (user -> vault)
            $reason = 'Bridge: Transfer to Ethereum (' . $this->cfg['ethereum']['network'] . ')';
            ...
            if (($resp['status'] ?? 'error') !== 'success') {
                ...
                Logger::error('Ethereum withdrawal YEMChain voucher failed', ['uid' => $uid, 'amount' => $amount, 'error' => $errorMsg]);
                ...
                if ($feeEnabled && isset($feeResp['txnID'])) {
                    ...
                    $refundResp = $this->yem->createVoucher([
                        ...
                        'currencyCodeFrom' => 'USD',
                        'currencyCodeTo' => 'USD',
                        'reason' => 'Refund: Ethereum withdrawal fee (DBV transfer failed)',
                    ]);
                    ...
                    if (($refundResp['status'] ?? 'error') !== 'success') {
                        ...
                        // Still return error to user, but log that refund failed
                    } else {
            ...
            $yemHash = $resp['txnID'] ?? '';
            if (empty($yemHash)) {
                Logger::error('Ethereum withdrawal - YEMChain transaction ID missing', ['uid' => $uid, 'amount' => $amount]);
                ...
                if ($feeEnabled && isset($feeResp['txnID'])) {
                    ...
                    $refundResp = $this->yem->createVoucher([
                        ...
                        'currencyCodeFrom' => 'USD',
                        'currencyCodeTo' => 'USD',
                        'reason' => 'Refund: Ethereum withdrawal fee (transaction ID missing)',
                    ]);
        ...
        try {
            ...
            if ($this->hasReferralColumns('ethereum_withdraw')) {
            ...
            $ins = $this->pdo->prepare("INSERT INTO ethereum_withdraw ($insertCols) VALUES ($insertVals)");
            $ins->execute($insertParams);
            
            if ($ins->rowCount() === 0) {
                Logger::error('Ethereum withdrawal - Failed to insert record', ['uid' => $uid, 'amount' => $amount, 'address' => $address]);
            ...
            if (!$withdrawalId || $withdrawalId <= 0) {
                Logger::error('Ethereum withdrawal - Invalid withdrawal ID', ['uid' => $uid, 'amount' => $amount]);
            ...
            if (!$manualWithdrawEnabled) {
                try {
                    ...
                    Logger::warning('Ethereum withdrawal - Worker trigger failed', ['withdrawal_id' => $withdrawalId, 'error' => $e->getMessage()]);
                }
            } else {
                Logger::info('Ethereum withdrawal - Manual mode: skipping worker trigger', ['withdrawal_id' => $withdrawalId, 'uid' => $uid]);
            }
            
            $msgManual = $manualWithdrawEnabled ? ' Your withdrawal request is being processed. The process can take up to 24 hours to complete.' : ' Your DBV tokens will be sent to your Ethereum address automatically within seconds.';
            echo json_encode([
                'success' => true,
            ...
            Logger::error('Ethereum withdrawal - Database error', ['uid' => $uid, 'amount' => $amount, 'error' => $e->getMessage()]);
            echo json_encode(['success' => false, 'message' => 'Database error occurred. Please contact support.']);
            return;
        } catch (Exception $e) {
            Logger::error('Ethereum withdrawal - Unexpected error', ['uid' => $uid, 'amount' => $amount, 'error' => $e->getMessage()]);
    ...
    {
        // Try HTTP trigger first (port 3003 for Ethereum)
        $httpPort = 3003;
        ...
        curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode(['id' => $withdrawalId, 'network' => 'ethereum']));
        ...
        if ($error || !$result) {
            ...
            if ($secret !== '') {
                ...
                curl_setopt($ch2, CURLOPT_POSTFIELDS, http_build_query(['id' => $withdrawalId, 'network' => 'ethereum']));
