<?php
// Secure endpoint to update Ethereum withdrawal status
...
if ($hashOnly && $hash) {
    $stmt = $pdo->prepare('UPDATE ethereum_withdraw SET txn_hash_eth = :h, error_message = NULL WHERE id = :id');
    $result = $stmt->execute(['h' => $hash, 'id' => $id]);
} elseif ($hash) {
    $stmt = $pdo->prepare('UPDATE ethereum_withdraw SET status = :s, txn_hash_eth = :h, error_message = NULL WHERE id = :id');
    $result = $stmt->execute(['s' => $status ?: 3, 'h' => $hash, 'id' => $id]);
} elseif ($status === 2) {
    $stmt = $pdo->prepare('UPDATE ethereum_withdraw SET status = 2, error_message = :e WHERE id = :id');
    $result = $stmt->execute(['e' => $errorMessage ?: null, 'id' => $id]);
} else {
    $stmt = $pdo->prepare('UPDATE ethereum_withdraw SET status = :s, error_message = NULL WHERE id = :id AND status != :s2');
