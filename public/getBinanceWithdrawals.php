<?php
// Secure worker endpoint: returns Binance withdrawals by status in last 24h
...
if ($status === 0 && $includeStuck) {
    ...
    $stmt = $pdo->prepare('SELECT * FROM binance_withdraw WHERE status = :s' . $manualFilter . ' AND created_at > :c LIMIT 0,10');
